...
function draw() {
  ...
  if(pose){
    refreshPoints().then((kp)=>{
      ...
      if(pPoints.length<=0 && kp.length > 0) pPoints = kp;
      rawPoints = kp;

      pPoints.forEach((item, i) => {
        var s = .3;
        item.position.x = lerp(item.position.x, kp[i].position.x, s)
        item.position.y = lerp(item.position.y, kp[i].position.y, s)
      });
    ...
    if(rawPoints && pPoints){
      // drawSimpleCircles(rawPoints)
      // drawStick();
      // drawSpots(pPoints, rawPoints);
      drawStars();
      // drawBlob();
      // drawShader();
      // drawOrganic(pPoints);
...
function drawOther(data) {
  drawSimpleCircles(data.raw)
  // drawStick();
  drawSpots(data.smooth, data.raw);
  // drawStars();
  // drawBlob();
  // drawShader();
  // drawOrganic(data.smooth);
}
